...
WHERE current.player_username = 'player who login' AND current.player_username != other.player_username ;
'''
...
FROM (
    ...
    FROM playerinmatch AS current
    JOIN playerinmatch AS other ON current.session_id = other.session_id
    WHERE current.player_username = 'a_aykac' AND current.player_username != other.player_username
